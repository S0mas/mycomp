...
class TestWaitForTunnel:
    def test_returns_true_on_successful_response(self):
        from aicompany.cli import _wait_for_tunnel
        with patch("aicompany.cli.urllib.request.urlopen"):
            assert _wait_for_tunnel("https://example.trycloudflare.com", timeout=5) is True
    ...
    def test_returns_false_when_tunnel_never_responds(self):
        from aicompany.cli import _wait_for_tunnel
        with patch("aicompany.cli.urllib.request.urlopen", side_effect=OSError("refused")), \
             patch("aicompany.cli.time.sleep"):
            assert _wait_for_tunnel("https://example.trycloudflare.com", timeout=0) is False

    def test_retries_until_tunnel_responds(self):
        import urllib.error
        from aicompany.cli import _wait_for_tunnel
        calls = [OSError("not yet"), OSError("not yet"), None]
        def fake_urlopen(url, timeout=None):
            e = calls.pop(0)
            if e:
                raise e
        with patch("aicompany.cli.urllib.request.urlopen", side_effect=fake_urlopen), \
             patch("aicompany.cli.time.sleep"):
            assert _wait_for_tunnel("https://example.trycloudflare.com", timeout=10) is True

    def test_auto_mcp_fails_when_tunnel_unreachable(self):
        from aicompany.cli import _auto_mcp_context
        with patch("aicompany.cli.subprocess.Popen") as mock_popen, \
             patch("aicompany.cli.time.sleep"), \
             patch("aicompany.cli.Path.open", MagicMock()), \
             patch("aicompany.cli.re.search") as mock_re, \
             patch("aicompany.cli._wait_for_tunnel", return_value=False):
            mock_popen.return_value.poll.return_value = None
            mock_re.return_value = MagicMock(group=lambda: "https://x.trycloudflare.com")
...
class TestMCPErrorNoRetry:
    def test_mcp_connection_error_is_not_retried(self):
        ...
        backend.call.side_effect = mcp_err
